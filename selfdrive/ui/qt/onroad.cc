...
void AnnotatedCameraWidget::drawLaneLines(QPainter &painter, const UIState *s) {
  ...
  if (scene.adjacent_path) {
    ...
    paintLane(painter, scene.track_adjacent_vertices[4], scene.lane_width_left, blindSpotLeft);
    paintLane(painter, scene.track_adjacent_vertices[5], scene.lane_width_right, blindSpotRight);
  }
...
void AnnotatedCameraWidget::drawLead(QPainter &painter, const cereal::RadarState::LeadData::Reader &lead_data, const QPointF &vd) {
  ...
  if (leadInfo) {
    ...
    float lead_speed = std::max(v_rel, 0.0f);  // Ensure lead speed doesn't go under 0 m/s cause that's dumb
...
void AnnotatedCameraWidget::updateFrogPilotWidgets(QPainter &p) {
  ...
  slcOverridden = scene.speed_limit_overridden && scene.speed_limit_controller;
  slcSpeedLimit = scene.speed_limit && scene.speed_limit_controller;
...
void Compass::paintEvent(QPaintEvent *event) {
  ...
  std::map<QString, std::tuple<QPair<float, float>, int, QColor>> directionInfo = {
    {"N", {{292.5, 67.5}, Qt::AlignTop | Qt::AlignHCenter, QColor()}},
    {"E", {{22.5, 112.5}, Qt::AlignRight | Qt::AlignVCenter, QColor()}},
    {"S", {{112.5, 247.5}, Qt::AlignBottom | Qt::AlignHCenter, QColor()}},
    {"W", {{247.5, 337.5}, Qt::AlignLeft | Qt::AlignVCenter, QColor()}}
  };
  ...
  for (auto &item : directionInfo) {
    ...
    int offset = (direction == "E") ? -5 : (direction == "W") ? 5 : 0;
    ...
    p.setOpacity((bearingDeg >= fmin(minRange, maxRange) && bearingDeg <= fmax(minRange, maxRange)) ? 1.0 : 0.2);
